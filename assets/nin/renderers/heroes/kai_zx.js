...
loadTextures({
    "layer1": "nin:kai/kai_zx",
    "layer2": "nin:kai/kai_zx",
    "mask": "nin:null",
    "sword": "nin:kai/sword",
    "null": "nin:null",
...
function init(renderer) {
    ...
    renderer.setItemIcons("kai/kai_zx_0", "kai/kai_zx_1", "kai/kai_zx_2", "kai/kai_zx_3");
...
function initEffects(renderer) {
    ...
    spinjitzu.anchor.ignoreAnchor(false)
...
function initAnimations(renderer) {
    ...
    .setCondition(entity => (getBlockOnLeft(entity) != 'minecraft:air' && getBlockInFront(entity) == 'minecraft:air' && entity.getData("nin:dyn/climb_bool") && !entity.isOnGround()));

    addAnimation(renderer, "WALL_RUN_RIGHT", "nin:wall_run_right")
    .setData((entity, data) => {data.load(0, entity.loop(6) * Math.abs(entity.motionX()) * Math.abs(entity.motionZ()))})
    .setCondition(entity => (getBlockOnLeft(entity) != 'minecraft:air' && getBlockInFront(entity) == 'minecraft:air' && entity.getData("nin:dyn/climb_bool") && !entity.isOnGround() && entity.getData("fiskheroes:moving")));

    addAnimation(renderer, "WALL_HOLD_LEFT", "nin:wall_hold_left")
    .setData((entity, data) => {data.load(0, entity.getData("nin:dyn/climb_bool"))})
    .setCondition(entity => (getBlockOnRight(entity) != 'minecraft:air' && getBlockInFront(entity) == 'minecraft:air' && entity.getData("nin:dyn/climb_bool") && !entity.isOnGround()));

    addAnimation(renderer, "WALL_RUN_LEFT", "nin:wall_run_left")
    .setData((entity, data) => {data.load(0, entity.loop(6) * Math.abs(entity.motionX()) * Math.abs(entity.motionZ()))})
    .setCondition(entity => (getBlockOnRight(entity) != 'minecraft:air' && getBlockInFront(entity) == 'minecraft:air' && entity.getData("nin:dyn/climb_bool") && !entity.isOnGround() && entity.getData("fiskheroes:moving")));

    addAnimation(renderer, "WALL_JUMP_UP", "nin:wall_climb")
    .setData((entity, data) => {data.load(0, entity.loop(6) * Math.abs(entity.motionY()))})
    ...
    addAnimationWithData(renderer, "kai.CHARGEDBEAM", "fiskheroes:aiming", "fiskheroes:beam_charging").setCondition(entity => (entity.getData("fiskheroes:beam_charging")));
